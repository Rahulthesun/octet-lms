...
const deleteSubtopic = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Subtopic deleted", id: deleted.id });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};

module.exports = { createSubtopic, getSubtopicById, getSubtopicsByChapter, updateSubtopic, deleteSubtopic };