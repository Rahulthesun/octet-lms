...
function StorageBar() {
  const total   = 60; // Total Storage in GB
  const storage = useContentTree();
  return (
    <div className="flex items-center gap-4 bg-white p-4 rounded-2xl">
      <div className="flex-1">
        <div className="flex justify-between text-sm mb-1">
          ...
          <span className="font-inter font-bold text-primary">
            {formatBytes(storage.totalVideoBytes)} / {formatBytes(total * 1024 * 1024 * 1024)}
          </span>
        </div>
        <div className="h-1 bg-gray-200 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${(storage.totalVideoBytes / (total * 1024 * 1024 * 1024)) * 100}%`,
            }}
          />
        </div>
      </div>
      <span className="text-sm text-gray-600 shrink-0">
        <span className="font-inter">
          {formatBytes(total * 1024 * 1024 * 1024 - storage.totalVideoBytes)}
        </span>{" "}
        GB free
      </span>
    </div>
...
export default function ContentPage() {
  ...
  const selectedPath = (() => {
    ...
    const subject = (subjects as BackendSubject[]).find(
      (s) => s.id === openSubjectId,
    ) ?? null;
    const chapter = (
      (chaptersMap[openSubjectId] ?? []) as BackendChapter[]
    ).find((c) => c.id === openChapterId) ?? null;
  ...
  const visibility = (() => {
    ...
    for (const subject of subjects as BackendSubject[]) {
      ...
      const anyCh = (
        (chaptersMap[subject.id] ?? []) as BackendChapter[]
      ).some((c) => visibleChapters.has(c.id));
  ...
  function toggleChapter(chapterId: string) {
    ...
    if (next) {
      ...
      // Preserve previously chosen type; default to "pdf" on first open
  ...
  function switchContentType(chapterId: string, ct: ContentType) {
    ...
    setSelectedItemId(null); // clear selection when flipping tab
  }
  ...
  // ── Upload (optimistic — wire to API when ready) ───────────────────────────
   async function handleUpload() {
    ...
    if (contentType === "pdf") {
      try {
        ...
        formData.append("title", base.title);
        formData.append("chapterId", base.chapter_id);
       ...
       const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL || "http://localhost:8000"}/api/content/pdf/upload`,
           {
            method: "POST",
            body: formData,
          }
        ...
        return; // prevents optimistic UI update below  
      }
      //RUNS If THE uPLOAD IS SUCCESSFUL
                                      ...
                                      {chapterOpen && (
                                        ...
                                        >
                                          <div>
                                            ...
                                            {/* PDF / Video toggle pills */}
                                            {anyLoaded && (
                                              <div className="flex items-center gap-1.5 pl-10 pr-4 py-2">
                                                {(
                                                  ["pdf", "video"] as ContentType[]
                                                ).map((ct) => (
                                                  <button
                                                    key={ct}
                                                    onClick={(e) => {
                                                    ...
                                                    className={`px-3 py-0.5 text-xs rounded-full border transition-colors hover:cursor-pointer ${
                                                      contentType === ct
                ...
                <div className="border border-gray-200 rounded-md p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-400">
                      {fmtSize(selectedFile.size_bytes)} ·{" "}
                      {selectedPath.contentType === "video"
                        ? "Video"
                        : "Document"}{" "}
                      · uploaded {fmt(selectedFile.created_at)}
                    </p>
                  </div>
                </div>
            ...
            <div className="p-4">
              ...
              <div className="bg-white shadow-sm p-4 rounded-2xl space-y-5">
                <h2 className="text-base font-medium text-gray-800">
                  Upload {selectedPath.contentType === "pdf" ? "PDF" : "Video"}{" "}
          ...
          >
            ...
            >
              ...
              <p className="text-base text-gray-600 mt-2">
                ...
                {deleteTarget.type === "pdf" ? "PDF" : "video"}.
              </p>
