...
function ApplicationCard({
 ...
 <span className="text-base border border-gray-200 px-2 py-0.5 text-gray-600">{app.grade}</span>
 {!isPending && (
 <span className="text-base px-2 py-0.5 border border-gray-200 text-gray-600 capitalize">
 {app.status}
 </span>
 )}
 </div>
 ...
 {[
 { label: '10th ID Card', sub: 'Identity proof' },
 { label: '10th Grade Paper', sub: 'Academic proof' },
 ].map(({ label, sub }) => (
 <div
 key={label}
 className="border border-dashed border-gray-300 flex flex-col items-center justify-center gap-1.5 py-3 bg-gray-50"
 >
 ...
 <button className="text-sm text-[#5e4075] underline underline-offset-2 hover:text-[#3d2652]">
 View Document
 </button>
 </div>
 ))}
...
interface FilterDropdownProps {
 ...
 setStatusFilter: (s: 'All' | 'active' | 'blocked') => void
}

function FilterDropdown({ gradeFilter, statusFilter, setGradeFilter, setStatusFilter }: FilterDropdownProps) {
 const [open, setOpen] = useState(false)
 const ref = useRef<HTMLDivElement>(null)

 const activeCount = (gradeFilter !== 'All' ? 1 : 0) + (statusFilter !== 'All' ? 1 : 0)
 ...
 const clear = () => { setGradeFilter('All'); setStatusFilter('All') }

 return (
 <div className="relative" ref={ref}>
 <button
 onClick={() => setOpen((o) => !o)}
 className={`flex items-center gap-2 px-4 py-2.5 border text-base transition-colors ${
 open || activeCount > 0
 ...
 Filter
 {activeCount > 0 && (
 <span className="w-5 h-5 bg-[#5e4075] text-white text-sm flex items-center justify-center font-inter leading-none">
 {activeCount}
 </span>
 )}
 ...
 className="absolute left-0 top-full mt-1.5 z-30 w-64 bg-white border border-gray-200 shadow-sm p-4"
 >
 {/* Grade */}
 ...
 className={`px-3 py-1 text-base capitalize transition-colors ${
 statusFilter === s
 ? 'bg-[#5e4075] text-white'
 : 'border border-gray-200 text-gray-600 hover:border-gray-400'
 }`}
...
export default function StudentsPage() {
 ...
 New Applications
 {pendingCount > 0 && (
 <span className="text-sm px-2 py-0.5 bg-[#5e4075] text-white font-inter">
 {pendingCount}
 </span>
 )}
 </button>
 <button
 onClick={() => { setActiveTab('database'); setSearch('') }}
 className={`flex items-center gap-2 px-5 py-2.5 text-base border-b-2 transition-colors -mb-px ${
 activeTab === 'database'
 ? 'border-[#5e4075] text-[#5e4075]'
 : 'border-transparent text-gray-500 hover:text-gray-800'
 ...
 >
 Student Database
 <span className="text-base px-2 py-0.5 border border-gray-200 text-gray-500 font-inter">
 {students.length}
 </span>
 </button>
 </div>
 ...
 className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-none text-base text-gray-800 placeholder-gray-400 outline-none focus:border-gray-400 bg-transparent"
 />
 </div>
 <FilterDropdown
 ...
 <div className="hidden lg:grid lg:grid-cols-[40px_1fr_110px_80px_110px_110px_80px_44px] gap-3 px-5 py-3 text-base text-gray-400 border-b border-gray-200 bg-gray-50">
 <span>#</span>
 <span>Student</span>
 <span>Roll</span>
 <span>Grade</span>
 ...
 <span className="text-base border border-gray-200 px-2 py-0.5 text-gray-600 w-fit hidden lg:block">{s.grade}</span>
 <span className="text-base text-gray-500 hidden lg:block">{s.batch}</span>
 <div className="text-right hidden lg:block">
 <span className="text-base font-inter text-[#5e4075]">{s.onlineAtt}%</span>
 {s.offlineAtt !== null && (
 <span className="text-base font-inter text-gray-400"> / {s.offlineAtt}%</span>
 )}
 </div>
 <span className={`text-base text-right hidden lg:block ${
 s.status === 'blocked' ? 'text-gray-500 line-through' : 'text-gray-600'
 }`}>
