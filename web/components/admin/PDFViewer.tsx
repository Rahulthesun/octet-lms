...
export function PdfViewer({ url, filename, className = "" }: PdfViewerProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Dynamic import keeps Next.js SSR happy
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Watermark baked directly into canvas pixels — can't be removed via DOM
  ...
  useEffect(() => {
    ...
    const obs = new ResizeObserver(async () => {
      const fit = await computeFitScale(pdfDoc, pageNum);
      setFitScale(fit);
    });
  ...
  // ── Keyboard navigation (arrow keys, while viewer is focused) ──────────────
  ...
  const nextPage = () => setPageNum((n) => Math.min(n + 1, numPages));
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-gray-300 mt-0.5">
            Set <code className="bg-gray-100 px-1 rounded">NEXT_PUBLIC_R2_PUBLIC_URL</code>
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-white border-b border-gray-100 shrink-0">
          ...
          <div className="flex items-center gap-1.5 flex-1 min-w-0">
            <svg className="w-3.5 h-3.5 text-red-400 shrink-0" viewBox="0 0 14 14" fill="none">
              <path d="M2.5 1A1.5 1.5 0 0 1 4 0h5.5l3 3V13A1.5 1.5 0 0 1 11 14.5H4A1.5 1.5 0 0 1 2.5 13V1z"
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            ...
            >
              <svg className="w-2.5 h-2.5" viewBox="0 0 8 8" fill="none">
                <path d="M5.5 1L2.5 4l3 3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
            <span className="text-[11px] text-gray-500 tabular-nums w-14 text-center select-none">
              {loading ? "…" : numPages === 0 ? "—" : `${pageNum} of ${numPages}`}
            </span>
            <button
              onClick={nextPage}
            ...
            >
              <svg className="w-2.5 h-2.5" viewBox="0 0 8 8" fill="none">
                <path d="M2.5 1l3 3-3 3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </div>
        ...
        >
          ...
          {!loading && !loadError && (
            <div className="relative shadow-2xl rounded-sm">
              {/* Page-turning spinner */}
              {rendering && (
                <div className="absolute inset-0 bg-white/50 flex items-center justify-center z-10 rounded-sm">
              ...
              <canvas
                ...
                // Inline style prevents any attempt to drag/save the canvas
